...
public final class GameObjectController {
    ...
    public void createPlayers(List<EGameObject> playerNames) throws SlickException {
        Integer count;
        int startX;
        int playerWidth = ResourceManager.get().getGameAnimator(EGameObject.PLAYER).getCurrentContainer().getAnimation(0).getWidth();

        if (!playerNames.isEmpty()) {
            count = 1;
            startX = 150;
            for (EGameObject type : playerNames) {
                this.createPlayer(type, "player:" + count, startX, 200, 632, 0, true);
                startX += playerWidth - 10;
                ++count;
            }
        }
        this.updateOwnerCameraPlayer();
